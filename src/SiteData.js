...
function tableToArray(table) {
  ...
  if (headerCells.length === 0) {
    rows[0].querySelectorAll('td').forEach(cell => {
      headers.push(trimText(cell.textContent.trim(), "("));
    });
  } else {
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    for (let j = cellIndexOffset; j < headers.length; j++) {
      const cell = cells[j];
      rowData[headers[j]] = cell ? trimText(cell.textContent.trim() , "(") : '';
    }
...
export async function scrapeCDCDiabetesData() {
  ...
  tables.forEach((table, index) => {
    ...
    let tableCaption = "Table " + (index + 1) + " on CDC Diabetes Data site";
